...
#ifdef USE_HEAVY
static char *opt_set_vote(const char *arg, void *p)
{
    int t;
    uint16_t ts;
    char tc;
    char* result = set_int_range(arg, &t, 0, 1024); 
    if (result)
        t = 512;
    ts = (uint16_t) t;
    ts = be16toh(ts);
    opt_vote[1] = ts >> 8;
    opt_vote[0] = (unsigned char)(ts & 0xF);
    opt_vote_hex = bin2hex(opt_vote, 2);
    tc = opt_vote[0]; opt_vote[0] = opt_vote[1]; opt_vote[1] = tc;
    return NULL;
}
#endif
...
#ifdef USE_HEAVY
    if (opt_heavy) {
       work->data[83] = opt_vote[0]; 
       work->data[82] = opt_vote[1]; 
       work->data[81] = pool->reward_bin[1]; 
       work->data[80] = pool->reward_bin[0]; 
    }
#endif
