...
_clState *initCl(unsigned int gpu, char *name, size_t nameSize)
{
    ...
    if (cgpu->kernel == KL_NONE) {
        if (opt_scrypt) {
            ...
            applog(LOG_INFO, "Selecting skein kernel");
            clState->chosen_kernel = KL_SKEIN;
        } else if (opt_heavy) {
    ...
    if (cgpu->cl_filename) {
        ...
        switch (clState->chosen_kernel) {
            ...
            case KL_SKEIN:
                ...
                break;
            case KL_HEAVY:
                strcpy(filename, HEAVY_KERNNAME".cl");
...
#ifdef USE_HEAVY
    if (opt_heavy) {
        clState->heavy_CLbuffer = clCreateBuffer(clState->context, CL_MEM_READ_ONLY, HEAVY_BUFFER_SIZE, NULL, &status);
        if (status != CL_SUCCESS) {
            applog(LOG_ERR, "Error %d: clCreateBuffer (heavy_CLbuffer)", status);
        return NULL;
        }
        clState->outputBuffer = clCreateBuffer(clState->context, CL_MEM_WRITE_ONLY, BUFFERSIZE, NULL, &status);
    } else
