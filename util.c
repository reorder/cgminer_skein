...
static bool parse_notify(struct pool *pool, json_t *val)
{
...
#ifdef USE_HEAVY
    if (opt_heavy)
        reward = json_array_string(val, 9);
#endif
    ...
    snprintf(header, pool->swork.header_len,
        // snprintf truncates workpadding, which should not be used anyway
        "%s%s%s%s%s%s%s",
...
#ifdef USE_HEAVY
    if (opt_heavy)
            if (unlikely(!hex2bin(pool->reward_bin, reward, 2)))
                quit(1, "Failed to convert reward to reward_bin in parse_notify:");
#endif
